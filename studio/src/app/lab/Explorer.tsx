...
import { SwitchLang } from '@/shared/Lang';
import { ExampleItem, examplesMap, flatExamples } from './examples';
...
export function Explorer(props: { lang: string, setCode: (code: string) => void, setExample: (example: string) => void }) {
  ...
  const examples: ExampleItem[] | undefined = examplesMap[props.lang]
  ...
  const examplesTree = useMemo(() => {
    function toTree(items: ExampleItem[]): DataNode[] {
