...
// import { useComputedSocketPositions } from '../shared/socket-positions'
// import { debugId } from '../shared/debugId'
...
export async function createEditor<ParseResult, N extends { type: string }, F extends N>(container: HTMLElement, language: Language<AreaExtra, ParseResult, N, F>) {
  ...
  reactPlugin.addPreset(ReactPresets.contextMenu.setup({
    ...
    customize: {
      ...
      common: () => UI.ContextMenu.Common,
      search: () => UI.ContextMenu.Search,
      subitems: () => UI.ContextMenu.Subitems
    }
  }))
  ...
  return {
    ...
    startStepByStep: async (path: string) => {
      const item = flatExamples(language.examples).find(ex => ex.path === path)
